...
async function step7_launcher(claudePath, tvPath, mcpDir) {
  ...
  const script = [
   ...
   `echo "  |    TradingView2Claude Connector  v${app.getVersion()}     |"`,
    ...
    '  exit 1',
    'fi',
    'CUSTOMER_NAME=$(grep -o \'"customer_name":"[^"]*"\' "$LICENSE_FILE" | cut -d\'"\' -f4)',
   `GAS_URL="${GAS_URL}"`,
    'NODE_BIN="' + nodeBin + '"',
    'LIC_RESPONSE=$("$NODE_BIN" -e "',
    'const https=require(\'https\');',
   `const key=\'$LICENSE_KEY\',gasUrl=\'${GAS_URL}\';`,
    'const body=JSON.stringify({action:\'validate\',license_key:key});',
    'function get(url){const u=new URL(url);',
    'const req=https.request({hostname:u.hostname,path:u.pathname+u.search,method:\'GET\'},res=>{',
    'let r=\'\';res.on(\'data\',d=>r+=d);res.on(\'end\',()=>{process.stdout.write(r);process.exit(0);});});',
    'req.setTimeout(10000,()=>{req.destroy();process.exit(0);});req.on(\'error\',()=>process.exit(0));req.end();}',
    ...
    'let r=\'\';res.on(\'data\',d=>r+=d);res.on(\'end\',()=>{process.stdout.write(r);process.exit(0);});});',
    'req.setTimeout(10000,()=>{req.destroy();process.exit(0);});req.on(\'error\',()=>process.exit(0));',
    'req.write(body);req.end();',
    '" 2>/dev/null)',
    'if [ -z "$LIC_RESPONSE" ]; then',
    '  echo "  Licenza:  $LICENSE_KEY"',
    '  [ -n "$CUSTOMER_NAME" ] && echo "  Titolare: $CUSTOMER_NAME"',
    '  echo "  Stato:    ⚠ Non verificata (offline)"',
    'elif echo "$LIC_RESPONSE" | grep -q \'"ok":true\'; then',
    '  GAS_NAME=$(echo "$LIC_RESPONSE" | grep -o \'"customer_name":"[^"]*"\' | cut -d\'"\' -f4)',
    '  DISPLAY_NAME="${GAS_NAME:-$CUSTOMER_NAME}"',
    '  echo "  Licenza:  $LICENSE_KEY"',
    '  [ -n "$DISPLAY_NAME" ] && echo "  Titolare: $DISPLAY_NAME"',
    '  echo "  Stato:    ✓ Attiva"',
    'else',
    '  echo "  Licenza:  $LICENSE_KEY"',
    '  [ -n "$CUSTOMER_NAME" ] && echo "  Titolare: $CUSTOMER_NAME"',
    '  echo "  Stato:    ✕ Non attiva"',
    '  echo ""',
